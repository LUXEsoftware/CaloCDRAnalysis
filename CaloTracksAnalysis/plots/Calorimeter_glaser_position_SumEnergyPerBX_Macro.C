...
void Calorimeter_glaser_position_SumEnergyPerBX_Macro()
{
  SetLuxeStyle();
 ...
 // TFile* FileSig=TFile::Open("gamma_laser_w0_8000nm_phase0_nbx1943_final.root");
  ...
  TH1D* H_photon_sig  =(TH1D*)FileSig->Get("CaloPos_FrontPlate/CaloPos_FrontPlate_xp_energy_photon");
  TH1D* H_photon_bkg  =(TH1D*)FileBkg->Get("CaloPos_FrontPlate/CaloPos_FrontPlate_xp_energy_photon");
  TH1D* H_positron_sig=(TH1D*)FileSig->Get("CaloPos_FrontPlate/CaloPos_FrontPlate_xp_energy_positron");
  TH1D* H_positron_bkg=(TH1D*)FileBkg->Get("CaloPos_FrontPlate/CaloPos_FrontPlate_xp_energy_positron");
  TH1D* H_electron_sig=(TH1D*)FileSig->Get("CaloPos_FrontPlate/CaloPos_FrontPlate_xp_energy_electron");
  TH1D* H_electron_bkg=(TH1D*)FileBkg->Get("CaloPos_FrontPlate/CaloPos_FrontPlate_xp_energy_electron"); 
  


  H_photon_bkg->GetXaxis()->SetTitle("x position [mm]");
  H_photon_bkg->GetYaxis()->SetTitle("#sum E/BX [GeV]");
  
  H_photon_bkg->GetXaxis()->SetTitleOffset(1.4);
  H_photon_bkg->GetYaxis()->SetTitleOffset(1.4);
  H_photon_bkg->GetYaxis()->SetRangeUser(1e-4,3e3);
...
TLegend* leg=new TLegend(0.6,0.65,0.92,0.9);
  ...
  gPad->Print("Calorimeter_glaser_position_SumEnergyPerBX.png");
  gPad->Print("Calorimeter_glaser_position_SumEnergyPerBX.C");
...
#ifndef __CINT__
int main() { 
  Calorimeter_glaser_position_SumEnergyPerBX_Macro();
